...
const Web3StatusConnected = styled(Web3StatusGeneric)<{ pending?: boolean }>`
  background-color: ${({ pending, theme }) =>
    pending ? theme.primary1 : " rgba(255, 255, 255, 0.15)"};
...
border: 2px solid ${({ pending, theme }) =>
  pending ? theme.primary1 : theme.jediWhite};
    :focus {
...
function Web3StatusInner({ starkID }: { starkID?: string }) {
  ...
  const pending = sortedRecentTransactions
    .filter(
      tx =>
        !tx.receipt ||
        tx.receipt.status === "PENDING" ||
        tx.receipt.status === "RECEIVED"
    )
    .map(tx => tx.hash);
