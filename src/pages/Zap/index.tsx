...
export default function Zap() {
  ...
  const handleZap = (hash: string, tx: Transaction) => {
    ...
    if (
      hash &&
      txInfo &&
      txInfo?.toToken?.tokenInfo?.chainId ===
        txInfo?.fromToken?.tokenInfo?.chainId
    ) {
      const inputAmount = formatTransactionAmount(
        currencyAmountToPreciseFloat(txInfo?.inputAmount)
      );
      const outputAmount = formatTransactionAmount(
        currencyAmountToPreciseFloat(txInfo?.outputAmount)
      );
  ...
  useEffect(() => {
    if (!snChainId || snChainId === StarknetChainId.MAINNET) {
      ...
      getSupportedTokens({ chainId: [15366], protocol: ["jediswap.xyz"] }).then(
        tokens => {
          setToTokens(
            tokens.filter(token => {
      ...
      getSupportedTokens({ chainId: [15367], protocol: ["jediswap.xyz"] }).then(
        tokens => {
          setToTokens(
            tokens.filter(token => {
              const formattedToken = isAddress(token.address);
    ...
    <>
      <AutoColumn gap="14px" style={{ maxWidth: 470, padding: "2rem" }}>
        ...
        <HeaderInfo fontSize={16}>
          ...
          Thanks to Wido, it also supports tokens on Ethereum!
        </HeaderInfo>
      </AutoColumn>
      <StyledAppBody>
