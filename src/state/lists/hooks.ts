...
export class WrappedLPTokenInfo extends LPToken {
  ...
  constructor(
    ...
    const tokens = token0.sortsBefore(token1)
      ...
      : [token1, token0];
    const address =
      PAIR_ADDRESSES_CACHE?.[tokens[0].address]?.[tokens[1].address] ??
...
const lpListCache: WeakMap<TokenList, LPTokenAddressMap> | null =
  typeof WeakMap !== "undefined"
    ? new WeakMap<TokenList, LPTokenAddressMap>()
    : null;
...
export function listToLPTokenMap(
  list: TokenList,
  allPairs: string[]
): LPTokenAddressMap {
  ...
  const map = list.tokens.reduce<LPTokenAddressMap>(
    (tokenMap, tokenInfo) => {
      ...
      const bases: Token[] =
        BASES_TO_BUILD_ZAPPER_LIST_AGAINST[tokenInfo.chainId];

      const lpTokens: WrappedLPTokenInfo[] = bases.flatMap(baseToken => {
      ...
      const filteredLpTokens = lpTokens
        .filter(Boolean)
        .filter(lpToken => allPairs.includes(lpToken.address));
