...
async function fetchChunk(
  ...
  if (number.toBN(resultsBlockNumber).toNumber() < minBlockNumber) {
    console.debug(
  ...
  return {
    results: returnData,
    blockNumber: number.toBN(resultsBlockNumber).toNumber()
  };
}
...
export function parseReturnData(
  ...
  if (contractInterface) {
    ...
    const hasUint256Output = contractInterface.outputs.some(
      o => o.type === "Uint256"
    );
    const hasArrayOutput = contractInterface.outputs.some(o =>
      /_len$/.test(o.name)
    );
    ...
    if (hasMultipleOutputs) {
      ...
      if (!hasUint256Output) {
        ...
        const parsedReturnData = outputAbiEntries.reduce<{
      ...
      if (!hasUint256Output) {
        ...
        const parsedReturnData = number.toHex(
          uint256.uint256ToBN(uint256Result)
        );
              ...
              updateMulticallResults({
                ...
                results: outdatedCallKeys
                  ...
                  .reduce<{ [callKey: string]: string | null }>(
                    (memo, callKey, i) => {
                      const methodAbi =
                        debouncedListeners?.[chainId]?.[callKey]?.methodAbi;
